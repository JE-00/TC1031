...
void leerArchivo(vector <bitacora*> &registros) {
    ...
    ifstream archBitacora("bitacora.txt");
    ...
    if (archBitacora.is_open()) {
        ...
        // Se cierra el archivo bitacora.txt para evitar daÃ±os
        archBitacora.close();
    }
}
...
void guardarArchivo(vector<bitacora*> registros) {
    ofstream archNuevaB("bit.txt");
    if (archNuevaB.is_open()) {
        // En un ciclo de 0 al tamaÃ±o del vector registros se imprimen los atributos de cada elemento separados por un espacio
...
int main() {
    ...
    // Llama a la funciÃ³n leerArchivo
